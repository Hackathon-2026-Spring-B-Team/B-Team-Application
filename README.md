...
## デザイン制作
デザインはユーザーがどこでも勉強ができるようスマホアプリの画面を想定していたため、有名かつモダンなアプリを参考にしました。<br>
また、UI原則に基づくデザインを実装するためにSparkle Designを利用し開発効率とUI品質向上、デザインの一貫性を心がけました

### 参考とツール
**参考にしたアプリ:**
- メルカリ,三菱UFJ銀行,Duolingo,YouTube,PayPay,LINE,etc...

**デザインに使用したツール:**
- Figma: デザインを実際に作る
- Sparkle Design: UI原則に基づき実装するためコンポーネントなどを利用

### UI/UXにおいて意識した点
**UI**
- 勉強アプリのため画面の色を目に優しい深い緑色(`#116329`)を基調とし一貫性を保った
- 戻るボタン(`>`)や中断ボタン(`×`)、カードなどを実装しユーザーが動きを予測できるようにした
- ナビゲーションバーにより画面の遷移が用意にできる

**UX**
- ホーム画面に本日のタスクのみが表示されることでユーザーが今日やることだけをすぐに知ることができる
- リンク画面から登録済みの勉強サイトや動画に移動できるため余計な情報をブラウザで見なくて済む
- 理解度別および期間別のタスクフィルタリングと理解度別グラフによりプランの状況を把握することができる
- 削除や中断などの重要な動作にモーダルを実装し、ユーザーに警告の表示と確認を促すようにした